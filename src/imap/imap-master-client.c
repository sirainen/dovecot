				...
				const char **error_r)
{
	...
	if (auth_token == NULL || session_pid == NULL) {
		*error_r = "Missing auth_token or session_pid";
		return -1;
	}
	...
	token_payload = t_str_new(128);
	str_append(token_payload, "imap");
	str_append_c(token_payload, '\0');
	str_append(token_payload, session_pid);
	str_append_c(token_payload, '\0');
	str_append(token_payload, username);
	str_append_c(token_payload, '\0');
	str_append(token_payload, session_id != NULL ? session_id : "");
	str_append_c(token_payload, '\0');
